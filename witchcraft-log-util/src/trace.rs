...
/// Information about the native module containing this crate.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ModuleTraceInfo {
    ...
    /// The platform-specific build ID, in findshlibs' canonical string form.
    pub build_id: Option<String>,
}

/// Return trace information for the native module this crate is linked into, or `None` if it
...
pub fn find_module_trace_info() -> Option<ModuleTraceInfo> {
    use findshlibs::{IterationControl, SharedLibrary, TargetSharedLibrary};

    let marker = find_module_trace_info as *const () as usize;
    let mut found = None;
    TargetSharedLibrary::each(|shlib| {
        ...
        found = Some(ModuleTraceInfo {
            address_start: base,
            address_len: shlib.len(),
            build_id: shlib.id().map(|id| id.to_string()),
        });
...
mod tests {
    ...
    #[test]
    fn finds_containing_module() {
        let info = find_module_trace_info().unwrap();
        let marker = find_module_trace_info as *const () as usize;

        assert!(info.address_len > 0);
        assert!(marker >= info.address_start);
        assert!(marker < info.address_start + info.address_len);
    }
}
