...
pub fn from_record(record: &Record<'_>) -> ServiceLogV1 {
    ...
    if let Some(error) = record.error() {
        ...
        // Append the installed trace context (module load base + version) so a stripped trace is
        // self-contained: the base turns each frame IP into a file-relative address, and the version
        // selects the artifact to resolve against. See `witchcraft_log::set_trace_context`.
