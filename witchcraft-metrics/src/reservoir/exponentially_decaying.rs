...
impl ExponentiallyDecayingReservoir {
    ...
    #[deprecated(
        note = "Use ExponentiallyDecayingReservoir::builder instead",
        since = "1.1.0"
    )]
...
/// A builder for [`ExponentiallyDecayingReservoir`].
pub struct Builder {
    clock: Arc<dyn Clock>,
    exemplar_provider: Arc<dyn Fn() -> Option<Arc<dyn Exemplar>> + Sync + Send>,
}
...
impl Builder {
    ...
    /// Sets the provider used to associate exemplars with each measurement.
    #[inline]
...
impl Reservoir for ExponentiallyDecayingReservoir {
    fn update(&self, value: i64) {
        self.histogram
            .lock()
            .update_at_ex(self.clock.now(), value, (self.exemplar_provider)());
    }
